...
async def tu_endpoint(msg: str = Form(...), image: UploadFile = File(...)):
    ...
    response = requests.post("http://ctf-backend-esteganografia:8000/encode", data=form_payload, files=file_payload)
    response_storage = requests.post("http://ctf-backend-storage:8000/upload", data=form_payload, files=file_storage_payload)
...
async def tu_endpoint(image: UploadFile = File(...)):
    ...
    response = requests.post("http://ctf-backend-esteganografia:8000/decode", files=file_payload)
